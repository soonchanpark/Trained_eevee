...
exit(0)
w_before = pipe.vace.vace_blocks[0].cross_attn.q.weight.detach().clone()

pipe.load_lora(pipe.vace, "./checkpoints/step-10000.safetensors", alpha=1)

w_after = pipe.vace.vace_blocks[0].cross_attn.q.weight.detach()
print("weight changed:", not torch.equal(w_before, w_after))  # True면 LoRA 적용됨
